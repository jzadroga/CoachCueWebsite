        ...
        public static twitteraccount Save(int? accountID, string twitterID, string twitterUsername, string twitterName, string profileImageUrl, int accountType, string statusType, string description)
        {
            ...
            try
            {
                ...
                if (twtAcnt == null)
                {
                    ...
                    twtAcnt.dateModified = DateTime.Now;
            ...
            if (users.Count() > 0)
            {
                foreach( var user in users ){
                    accounts.Add(new twitteraccount
                    {
                        ...
                        twitterID = user.Identifier.ID,
                        twitterUsername = user.Identifier.ScreenName,
            ...
            try
            {
                if (playerAccount.twitterAccountID.HasValue)
                {
                    ...
                    if (plyAcnt != null)
                    {
                        ...
                        twitteraccount.Save( playerAccount.twitterAccountID, playerAccount.twitteraccount.twitterID, twitAcnt.Identifier.ScreenName, twitAcnt.Name, twitAcnt.ProfileImageUrl, 1, "Active", twitAcnt.Description);
                    }
                }
                else
                {
                    ...
                    if (users.Count() > 0)
                    {
                        foreach (var user in users.ToList())
                        {
                            ...
                            if (userName.Count() > 1)
                            {
                                ...
                                {
                                    twitteraccount twtAcnt = twitteraccount.Save(null, user.Identifier.ID, user.Identifier.ScreenName, user.Name, user.ProfileImageUrl, 1, "Active", user.Description);

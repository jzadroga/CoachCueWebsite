...
namespace CoachCue.Tests
{
    ...
    public class messageTests
    {
        ...
        public void Create()
        {
            message newMessage = message.Save(1, 2919, "This is a test");
            if( newMessage.messageID == 0 )
                Assert.Fail("Message not created");

            Assert.Pass("MessageID: " + newMessage.messageID.ToString());
        }

        [Test]
        public void GetStream()
        {
            List<StreamContent> streamList = stream.GetStream(1);
